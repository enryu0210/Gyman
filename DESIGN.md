...
- 회원 상세 계약·최근수업 섹션 목업(라이트/다크, 2026-07-16): https://claude.ai/code/artifact/7fb9848d-07b3-45ae-8c18-c5ed300dd7f6
...
| 2026-07-16 | 회원 상세 계약·최근수업 섹션 정렬 | 계약: 제목에 묻혀 있던 "잔여 N회"를 `headlineMedium` w800 **큰 숫자**로 승격(트레이너 최다 조회 값), 잔여/총 비율을 **볼트 세션 게이지**로(DESIGN.md 승인 fill 용도). 활성=강조색·소진=회색. 최근수업: 상태 배지의 `Colors.amber/red/orange.shade100`(다크 카드 위 밝은 블록 함정)을 `renewalAlertStyle`처럼 밝기별 분기로 교체, 통증 칩 `Colors.deepOrange`·삭제 `Colors.red`도 동일 정리. |
| 2026-07-16 | 세션 상태 배지 공용화(`SessionStatusBadge`) + 예약 화면 정렬 | 상태→색 매핑이 예약 화면·최근수업에 **복제**돼 한쪽만 고치면 톤이 어긋나던 것을 `session_log/session_status_badge.dart` 한 곳으로 추출(밝기별 분기 내장). 예약 화면: 승인대기 앰버 블록(`Colors.amber.shade50/200/900`) 다크 대응 분기, 로컬 `_ErrorView`/`_EmptyView` → 공용 `AppErrorView`/`AppEmptyView`. 상세 잔여 카드(인바디·수업영상) 삭제 버튼 `Colors.red` → `error`. 이로써 **트레이너 측 하드코딩 색 부채 소진**(남은 `renewal_alerts_card`는 이미 밝기 분기된 기준 패턴). |
