...
### 3.10 회원 홈의 정보 우선순위 (UI 1차 개편 §3.3 완료)
- **순서는 "운동 경험 → 잔여 횟수"**: ① 오늘의 운동/다음 PT → ② 트레이너 메시지·안내 →
  ③ 이번 주 운동 현황 → ④ 최근 변화 → ⑤ 잔여 횟수·바로가기. 잔여 횟수를 위로 되돌리지
  말 것 — 회원이 앱을 여는 이유는 "오늘 운동을 어떻게 하지"이지 "몇 회 남았지"가 아니다.
  대신 잔여 횟수는 히어로의 부제로 한 번 더 보여 스크롤 없이 확인된다.
  회귀 테스트로 y 좌표를 고정해 뒀다(`test/features/member_home_layout_test.dart`).
- **볼트 강조는 화면에 하나, 주인은 상황이 정한다.** 오늘 수업이 진행 중/남았으면 히어로가,
  예약이 아예 없으면 히어로의 "예약 신청"이, 그 밖에는 격자의 "셀프 운동 기록"이 가져간다.
  판정은 `MemberHomeFocus.takesVoltAccent` 한 곳 — 위젯에서 조건을 새로 짜지 말 것.
- **회원의 "기록 대기"는 처리할 일이 아니라 기다림이다.** 트레이너 홈에선 수업 시간이
  지났는데 미기록이면 처리할 일로 세우지만, 회원 홈은 같은 상태를 "오늘 운동을 마쳤어요"로
  말하고 행동을 요구하지 않는다(`MemberFocusKind.todayFinished`). 진행 중/예정 판정 자체는
  트레이너와 **같은** `domain/today_schedule.dart` 를 재사용 — 두 역할의 기준이 갈리면 안 된다.
- **인바디 증감을 좋다/나쁘다로 칠하지 않는다.** 체중 -1kg 은 감량 회원에겐 성과, 증량
  회원에겐 반대다. 목표(`member_profiles.goal`)가 자유 텍스트라 기계 판정이 불가능하므로
  부호와 숫자만 중립 표기한다(의료 안전선). 성패 색을 넣자는 요구가 오면 이 항목부터 볼 것.
- **"이번 주 운동 목표"는 현황으로 대체했다.** 주당 목표 횟수를 담을 컬럼이 없어(1차 개편은
  스키마 변경 범위 밖) 임의 상수로 달성/미달을 판정하지 않는다. 목표 기능은 스키마가 생긴 뒤
  `domain/weekly_activity.dart` 에 분모를 더하는 방식으로 확장.
- **홈은 인바디를 직접 가볍게 읽는다.** 변화 추이 화면의 `myProgressProvider` 는 done 수업
  200건 + 운동 기록까지 끌어오므로 홈에서 재사용하지 않는다. `member_home_repository` 가
  `body_measurements` 5행만 조회한다.
