...
|---|---|---|---|
| **"다리가 빠진다"** (무릎 내반, knee valgus) | 정면 영상에서 고관절–무릎–발목 정렬각(**FPPA**, Frontal Plane Projection Angle) | **부분 가능** | 스포츠과학에서 2D 비디오 FPPA 로 통용되는 지표. 단 **카메라가 정면·고정**이어야 하고, 스쿼트 하강 최저점 등 **특정 구간**에서만 유효 |
...
- **L1-a. 제약 등록** — `member_conditions` 마이그레이션 + 트레이너 등록 카드. 회원 상세에 표시. *큐 없이 등록·조회부터.*
...
- **L3. 자동 검출(R&D)** — §2에서 "가능" 판정된 FPPA(무릎 내반)부터. 체형분석 B(ML Kit)·4.6 프레임추출 인프라 위에서. 결과는 **트레이너 후보 제시까지만.**
...
1. **제약 코드 목록**: 기본 세트를 무엇으로 할지(측만증/골반전방경사/라운드숄더/거북목/평발/무릎내반/어깨충돌…). 트레이너와 함께 확정 — **실제로 쓰는 것만** 넣어야 한다.
