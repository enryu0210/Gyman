...
| **1. 4.8 L1 제약 등록 + 큐** | ⏳ **L1-a 구현 완료(2026-07-25)** — `member_conditions`(0036) + 도메인·테스트 + 트레이너 등록 카드. **남은 것: L1-b** 동작패턴 8종 도메인 + `condition_coaching_rules` 시드 → 셀프기록·수업기록에 큐 표시 | **0** | 1 (0036 적용 대기) | **② 체형 제약 전부** |
...
- [ ] **`0036_member_conditions.sql` SQL Editor 적용**(2026-07-25 추가, 4.8 L1-a) → 파일 하단 검증 SQL 5종 실행. 특히 ⑶ `source` CHECK 가 잘못된 값을 막는지, ⑷ 같은 회원+코드 활성 중복이 차단되는지, ⑸ 회원 계정에서 타인 제약이 안 보이는지. **적용 전까지 회원 상세의 "체형 특이사항" 카드는 조회 에러가 난다**(테이블 부재).
