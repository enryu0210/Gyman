...
| **4.8** | **동작 습관·체형 제약 코칭** — ⬜ **설계만 완료**(신규, 2026-07-24 피드백 / 07-25 정정) | **4.5(자세 영상 분석 R&D)를 흡수·구체화.** 요구 = "다리가 빠진다" 같은 **동작 습관** + "측만증으로 인한 운동 변경" 같은 **체형 제약**. **결정: 앱은 판단하지 않고 트레이너의 판단을 구조화해 나른다.** 3층 — L1 제약 등록(출처 `medical`/`trainer_observation` 강제 구분) → 동작 패턴 8종별 큐 표시 / L2 트레이너가 영상 시점에 마킹 → 회원 재생 시 재생 / L3 자동 검출(후순위·보조). **L1·L2는 CV 0·신규 의존성 0.** ⚠ 자동 검출 가능 범위는 설계 §2 표로 고정(무릎 내반=부분가능 / 체중분포=측정불가 / 측만증=의료영역). 상세: `docs/design_movement_coaching.md` |
...
| **1. 4.8 L1 제약 등록 + 큐** | `member_conditions` + 동작패턴 8종 도메인·테스트 + `condition_coaching_rules` 시드 → 셀프기록·수업기록에 큐 표시 | **0** | 1 | **② 체형 제약 전부** |
...
| **6. 확장** | 4.8 L3 자동 검출(FPPA 무릎 내반부터) + 고스트 G2/G3 + 영상 트래킹 4.6 | **0**(재사용) | 0~1 | 엔진 공유 회수 |
