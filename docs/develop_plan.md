...
>
> 작성일: 2026-05-24 (최종 갱신: 2026-05-29 — 진행 현황 반영)
> 대상 브랜치: `develop`
...
### ⏳ 진행/검증 중
- Edge Function(1.9~1.11): 배포 + `LLM_API_KEY` 설정 후 실사용 검증 단계.
- 회원 앱: 계정 연결까지 완료, 이후 기능은 아래 로드맵대로 진행.
...
- **회원 앱 기능**(§4 Phase 2에서 당겨옴): ✅ ② 회원 홈(다음 수업·잔여) / ✅ ③ 내 기록 열람
  ...
  / ✅ ⑦ 회원 채팅(S2 / 2.3, 1:1 실시간 — 0024 messages RLS·실시간).
- Phase 2 정착(피드백·셀프기록·FAQ·알림시간대) → Phase 3 관리자 → Phase 4 카메라/체형 AI.
...
| 2.5 | ✅ 회원 셀프 운동 기록 간단 입력 (S4) | 초간단 3필드(날짜+운동 한 줄+통증·컨디션 메모). `self_workout_logs`(0028) — member_id `DEFAULT current_member_profile_id()` 로 회원이 id 안 넘겨도 INSERT, 위변조는 RLS WITH CHECK 차단. **가시성:** 회원 rw(`self_log_member_rw`) / **트레이너 read-only**(`self_log_trainer_read`, 통증 내역 보고 수업 반영). 공용 `features/self_log/`(repo·providers) + 회원 작성 화면(`features/member/self_log/`, `/member/self-log`) + 트레이너 읽기 카드(회원 상세). |
