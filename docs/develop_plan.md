...
- **Phase 1.0 ~ 1.11** — 트레이너 앱 MVP + 기본 AI 기능:
  - 로그인·역할 분기(1.1) / 회원 CRUD·오프라인 등록(1.2) / 계약·잔여 횟수(1.3) /
    수업 기록(1.4) / 이전기록 복사·즐겨찾기(1.5) / 예약·노쇼·취소(1.6) / 재등록 알림(1.7, in-app)
  - 수업 전날 안내 자동 적재(1.8) / AI-B 메시지 초안(1.9) / AI-C 메모 초안(1.10) /
    LLM 연동·가드(1.11: PII 마스킹·동의·일일 한도·장애 폴백)
- **회원 온보딩** — 초대 코드 연결(회원가입 시 코드 필수, 0019/0020).
- **회원 앱 기능 ②~⑦** — ② 홈(다음 수업·잔여) / ③ 내 기록 열람 / ④ 받은 안내 + 트레이너 발송(sent)
  / ⑤ 예약 신청(회원 신청→트레이너 승인) / ⑥ 변화 추이 그래프(S1/2.2, 중량+인바디 — 0023)
  / ⑦ 회원 채팅(S2/2.3, 1:1 실시간 — 0024) + 사진 첨부(0026).
- **Phase 2 정착** — FAQ(S3/2.4) / 셀프 운동 기록(S4/2.5 — 0028, 컨디션 1~10 점수) /
  방해금지 시간(2.3 — 0025) / 수업 영상 보관·열람(2.7 — 0027, MVP+수업연결+개수상한).
- **운톡 P1**(2026-06-16) — 출석 달력(PT/셀프 색 구분)·출석 스트릭 + PT 시작 전 기기 로컬 알림.
- **Phase 3.1-A/B, 3.2-A** — 관리자 인프라(0029)·대시보드(0030)·센터 규정/PT FAQ 관리(0031/0032).
- **Phase 3.5(운톡 P0)** — 설정·익명화 탈퇴(0034 + `delete-account` 함수)·인앱 문의(0033)·약관/동의.
...
- **Phase 3.2-B** 회원 인수인계(트레이너 변경 재배정) / **3.3** 회원 앱 정식 분리 / **3.4** 친구네 센터 파일럿.
- **Phase 4 — 우선순위 상향(2026-07-24 피드백 반영).** 트레이너 피드백 2건("영상은 봐도 따라해지지 않는다 → 고스트 오버레이" / "개인화된 신체·습관 피드백")으로 **체형분석(4.2)을 앞당기기로 결정**. 착수 순서는 §4 Phase 4 하단 "CV·개인화 트랙 실행 순서" 참조.
...
| 4.2 | 체형 분석 (C3) — ⬜ **설계만 완료 · 🔺우선순위 상향(2026-07-24)** | **결정 고정: 100% 온디바이스**(ML Kit Pose, 사진 외부전송 0, LLM·비용 0). 좌우 어깨/골반 비대칭 우선. 첫 등록 + 4·8·12주 비교. ⚠ 착수 전 0008 스키마 빚(`body_assessments.member_id` user_id→id) 정리 필요. **CV 트랙의 관문** — 4.6·4.7이 ML Kit/`camera` 를 공유하므로 여기를 세우면 나머지 한계비용이 급감. 상세: `docs/design_body_analysis.md` |
...
| **4.8** | **개인화 피드백(신체·습관 리포트)** — ⬜ **설계만 완료**(신규, 2026-07-24 피드백) | 이미 쌓인 출석·통증·컨디션·중량·인바디를 한 사람 단위로 합쳐 **규칙기반**으로 인사이트 도출. **신규 수집·테이블·마이그레이션·의존성 전부 0.** 3층 가시성: 사실=회원(검수 불필요) / 판단=트레이너 전용 / 조언=기존 `outgoing_notifications` 검수 게이트 경유. LLM은 **문장 다듬기만**(수치·판정은 규칙기반 — 환각 차단). **CV 트랙과 독립 → 가장 먼저 착수 가능.** 체형분석 결과는 나중에 규칙 하나(`asymmetry`)로 합류. 상세: `docs/design_personal_insight.md` |

#### CV·개인화 트랙 실행 순서 (2026-07-24 피드백 반영)

> **피드백:** ① "운동영상을 본다고 해도 따라한다고 되는 게 아니다 — 카트라이더 고스트처럼 PT 영상을 투명도 낮춰 보는 느낌" ② "개인화된 신체·습관 피드백이 들어갔으면"
> **결론:** 체형분석(4.2)을 앞당긴다. 근거는 4.2가 **CV 트랙의 관문**이기 때문 — 4.2·4.6·4.7이 `google_mlkit_pose_detection` 과 `camera` 를 공유해, 4.2를 세우면 나머지 둘의 한계비용이 급감한다.

**순서 원칙: 의존성 추가가 늦을수록 좋다.** 네이티브 플러그인은 빌드·기기 호환 리스크라, 검증 안 된 앱에 먼저 얹으면 장애 원인 분리가 안 된다. 그래서 **의존성 0인 작업 → `camera` → ML Kit** 순으로 쌓는다.

| 단계 | 내용 | 신규 의존성 | 마이그레이션 | 회수하는 피드백 |
|---|---|---|---|---|
| **0. PoC 3종** | ① 카메라+영상 동시 렌더(4.7) ② ML Kit Pose 정지사진 + **APK 크기·빌드시간 측정**(4.2) ③ 영상 프레임 추출(4.6/G2 관문) | 임시 | 0 | — (리스크 제거) |
| **1. 개인화 피드백 PI-0~2** | 도메인 규칙+테스트 → 회원 리포트 → 트레이너 코칭 인사이트 | **0** | **0** | ② 대부분 |
| **2. 체형분석 A** | **0008 스키마 빚 정리**(`member_id` user_id→id) + RLS 재작성 + `body-photos` 버킷 + 동의 + `posture_metrics` 순수함수·테스트 | **0** | 1 | (관문 정리) |
| **3. 고스트 G1** | 반투명 합성 + 정렬·템포·반전 통제 + 정렬값 기기 저장 | `camera` | **0** | **① 전부** |
| **4. 체형분석 B·C** | ML Kit 연동 → 촬영 가이드(4.1) → 검수(4.3) → 회원 열람·4/8/12주 비교 | `google_mlkit_pose_detection` | 0 | ② 완성(신체 축) |
| **5. 확장** | 고스트 G2/G3(스켈레톤·자동정렬) + 영상 트래킹 4.6 + PI-4(`asymmetry` 합류) | **0**(재사용) | 0~1 | 엔진 공유 회수 |

- **PoC(0단계)에서 막히는 항목이 있으면 그 트랙만 보류**하고 나머지는 진행한다 — 세 PoC는 서로 독립적이다. 가장 싼 값에 가장 큰 불확실성을 제거하는 순서.
- **1·2단계는 의존성 0이라 §8 검증 백로그·1.12 베타 배포와 병행 가능.** 3단계 이후(네이티브 플러그인 도입)는 **베타 배포를 1회 내보낸 뒤** 착수를 권장 — 안 그러면 "베타에서 앱이 깨졌다"의 원인이 기능인지 플러그인인지 가려지지 않는다.
- ML Kit 도입(4단계)은 **APK 크기·빌드시간 측정 후 §0 의존성 표 갱신**이 선행 조건(3개 설계 문서 공통 경고).
...
| 데이터 모델 상세 | `docs/data_model.md` | 완료 (마이그레이션 0001~0020 반영) |
| 체형 분석 설계 (4.1~4.3) | `docs/design_body_analysis.md` | 설계 완료 · 미구현 |
| 수업 영상 시스템 (2.7) | `docs/design_class_videos.md` | MVP+B1/B2 구현 완료 |
| 영상 트래킹·자동 프레이밍 (4.6) | `docs/design_class_video_tracking.md` | 설계 완료 · 미구현 |
| 고스트 오버레이 (4.7) | `docs/design_ghost_overlay.md` | 설계 완료 · 미구현 (2026-07-25) |
| 개인화 피드백 (4.8) | `docs/design_personal_insight.md` | 설계 완료 · 미구현 (2026-07-25) |
| 와이어프레임 | `docs/wireframes/` | v0.1 작성 완료 |
| Edge Function 가이드 | `src/supabase/functions/README.md` | 완료 (배포·시크릿·함수 목록) |
| 마이그레이션 인덱스 | `src/supabase/migrations/README.md` | 완료 (0001~0020) |
