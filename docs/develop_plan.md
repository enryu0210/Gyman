...
| **1. 4.8 L1 제약 등록 + 큐** | ✅ **구현 완료(2026-07-25)** — `member_conditions`(0036) + 트레이너 등록 카드 / 동작패턴 8종 도메인 + `condition_coaching_rules`(0037, 시드 19건) → 회원 셀프기록·트레이너 수업기록에 큐 표시 | **0** | 2 (0036 ✅ / **0037 적용 대기**) | **② 체형 제약 전부** |
| **2. 4.8 L2 영상 시점 지적** | ✅ **구현 완료(2026-07-25)** — `class_video_marks`(0038) + 재생기 확장(진행바 눈금·시점 오버레이·목록 탭 이동) + 트레이너 마킹/삭제 | **0** | 1 (**0038 적용 대기**) | **② 동작 습관 전부**(정확도 100%) |
...
- [ ] **`0037`·`0038` SQL Editor 적용**(2026-07-25 추가, 4.8 L1-b/L2) — 적용 전엔 큐가 안 뜨고(규칙 0건, 에러는 없음) 영상 마킹은 조회 에러가 난다.
