...
>
> 작성일: 2026-05-24 (최종 갱신: 2026-07-25 — 마이그레이션 0001~0036 전부 적용 완료 반영)
> 대상 브랜치: `develop`
...
> 마이그레이션은 `0001~0036`, Edge Function은 `health`/`generate-message-draft`/`generate-memo-draft`/`delete-account`.
...
### ⏳ 진행/검증 중 (← 현재 최우선 병목)
- ✅ **마이그레이션 `0001~0036` 전부 SQL Editor 적용 완료**(0001~0034: 2026-07-06 / `0035`(안내 읽음 추적)·`0036`(회원 체형 특이사항): 2026-07-25). 남은 건 각 기능의 end-to-end RLS 동작 검증.
...
| **4.8** | **동작 습관·체형 제약 코칭** — ⏳ **L1-a 구현 완료**(2026-07-25, 0036) | **4.5(자세 영상 분석 R&D)를 흡수·구체화.** 요구 = "다리가 빠진다" 같은 **동작 습관** + "측만증으로 인한 운동 변경" 같은 **체형 제약**. **결정: 앱은 판단하지 않고 트레이너의 판단을 구조화해 나른다.** 3층 — L1 제약 등록(출처 `medical`/`trainer_observation` 강제 구분) → 동작 패턴 8종별 큐 표시 / L2 트레이너가 영상 시점에 마킹 → 회원 재생 시 재생 / L3 자동 검출(후순위·보조). **L1·L2는 CV 0·신규 의존성 0.** ⚠ 자동 검출 가능 범위는 설계 §2 표로 고정(무릎 모임=부분가능 / 체중분포=측정불가 / 측만증=의료영역). 상세: `docs/design_movement_coaching.md` |
...
| **1. 4.8 L1 제약 등록 + 큐** | ⏳ **L1-a 구현 완료(2026-07-25)** — `member_conditions`(0036) + 도메인·테스트 + 트레이너 등록 카드. **남은 것: L1-b** 동작패턴 8종 도메인 + `condition_coaching_rules` 시드 → 셀프기록·수업기록에 큐 표시 | **0** | 1 (0036 ✅ 적용 완료) | **② 체형 제약 전부** |
| **2. 4.8 L2 영상 시점 지적** | `class_video_marks` + 트레이너 마킹 UI + 회원 재생 시 표시 | **0** | 1 | **② 동작 습관 전부**(정확도 100%) |
| **3. 체형분석 A** | **0008 스키마 빚 정리**(`member_id` user_id→id) + RLS 재작성 + `body-photos` 버킷 + 동의 + `posture_metrics` 순수함수·테스트 | **0** | 1 | (관문 정리) |
...
### 검증 (배포·설정 후 동작 확인)
> ✅ 마이그레이션 `0001~0036` 전부 적용 완료(0035·0036은 2026-07-25). 아래는 **적용된 스키마 위에서 기능이 실제로 도는지** 확인하는 잔여.
...
| 동작 습관·체형 제약 코칭 (4.8, 4.5 흡수) | `docs/design_movement_coaching.md` | 설계 완료 · 미구현 (2026-07-25) |
| 와이어프레임 | `docs/wireframes/` | v0.1 작성 완료 |
| Edge Function 가이드 | `src/supabase/functions/README.md` | 완료 (배포·시크릿·함수 목록) |
| 마이그레이션 인덱스 | `src/supabase/migrations/README.md` | 완료 (0001~0036 — 전부 적용 완료 표기) |
