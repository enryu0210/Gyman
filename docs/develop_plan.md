...
>
> 작성일: 2026-05-24 (최종 갱신: 2026-07-25 — 마이그레이션 0001~0038 전부 적용 완료 반영)
> 대상 브랜치: `develop`
...
> 마이그레이션은 `0001~0038`, Edge Function은 `health`/`generate-message-draft`/`generate-memo-draft`/`delete-account`.
...
### ⏳ 진행/검증 중 (← 현재 최우선 병목)
- ✅ **마이그레이션 `0001~0038` 전부 SQL Editor 적용 완료**(0001~0034: 2026-07-06 / `0035`(안내 읽음)·`0036`(체형 특이사항)·`0037`(코칭 큐 규칙)·`0038`(영상 시점 지적): 2026-07-25). 남은 건 각 기능의 end-to-end RLS 동작 검증.
...
### 검증 (배포·설정 후 동작 확인)
> ✅ 마이그레이션 `0001~0038` 전부 적용 완료(0035~0038은 2026-07-25). 아래는 **적용된 스키마 위에서 기능이 실제로 도는지** 확인하는 잔여.
...
| 마이그레이션 인덱스 | `src/supabase/migrations/README.md` | 완료 (0001~0038 — 전부 적용 완료 표기) |
