...
| **3. 체형분석 A** | ✅ **구현 완료(2026-07-25)** — 0039(감사 컬럼·FK CASCADE·인덱스·`body-photos` 버킷·Storage RLS·`body_photo_consent`) + `posture_metrics`(ML Kit 비의존 순수 Dart) + `BodyAssessment` 모델(RLS 미러 게이트) | **0** | 1 (**0039 적용 대기**) | (관문 정리) |
...
- [ ] **`0039` SQL Editor 적용 + 동작 확인**(4.2 A단계) — 하단 검증 SQL 5종. 특히 ⑵ FK 가 CASCADE 인지, ⑶ `body-photos` 가 **비공개**인지, ⑸ **코멘트 없는 분석이 회원 계정에서 0건**인지(AI 단독 노출 차단 — 가장 중요).
