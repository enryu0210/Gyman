...
## 진행 상태 (2026-07-29 세션 — 중단 지점)

**환경**: Galaxy Z Flip 3 · 앱 재설치 완료(2026-07-29 11:41 빌드) · 트레이너 겸 관리자 로그인 중.
조작은 `adb shell input tap/swipe` + `screencap` 으로 자동화했다.

| 항목 | 결과 |
|---|---|
| 0.0 앱 최신화 | ✅ 구버전(07-21) 발견 → 재설치. 로그인·데이터 유지됨 |
| 0.1 계정 | ✅ 이미 준비돼 있었음 — 트레이너 겸 관리자(대시보드 타일 확인), 회원 "테스트" 앱 가입 완료 |
| 0.2 `ai_consent` | ✅ 이미 `true`(회원 목록 AI 칩 + 상세 배지) |
| 3.2 셀프 기록 트레이너 열람 | ✅ **통과** — 4건이 **읽기 전용**(자물쇠 아이콘, 수정·삭제 버튼 없음). RLS 과다 허용 아님 |
| L1-a 특이사항 **등록** | ✅ "척추측만 · 트레이너 관찰" + 회원 설명/트레이너 메모 저장. logcat 에러 0 |
| L1-a **해제** | ✅ 취소선 + "해제됨", 목록에는 남음(이력 보존) |
| L1-a **재적용** | ✅ "다시 적용했습니다" |
| L1-a **삭제** | ⏸ **여기서 중단** — 다음 세션에서 이어서 |

**남겨둔 데이터 (일부러 지우지 않음)**: 회원 "테스트" 에 특이사항 1건.
회원에게 보일 설명 `E2E-MEMBER-VISIBLE` / 트레이너 전용 메모 `E2E-TRAINER-ONLY`.
→ **회원 시점 검증에서 앞은 보이고 뒤는 안 보여야 한다**(가시성 분리 확인용).

**다음 세션에서 이어갈 것**
1. L1-a 삭제(확인 다이얼로그 → 삭제) — 라운드트립 완료
2. 회원 시점(라운드 1·2) — 로그아웃이 필요하므로 **회원 계정 로그인은 사용자가 직접**
3. SQL 검증 — `develop_plan.md:430-433`(0036~0039 하단 검증 SQL), 라운드 5 cron.
