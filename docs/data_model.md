...
> ---
> ## ⚠️ 아카이브 문서 — 내용이 낡았다 (2026-07-25)
> **이 문서를 스키마 정본으로 쓰지 말 것.** 마이그레이션 `0020` 까지만 반영돼 있어
> 현재(`0039`)와 어긋난다. 특히 **0013 이후 회원 식별자 정책이 바뀌었다**
> (`member_profiles.id` 가 PK, 회원 FK 는 전부 `id`) — 이 문서엔 그 이전 형태가 남아 있다.
>
> **스키마 정본 = `src/supabase/migrations/` 의 SQL 파일들** (목록: 그 폴더의 `README.md`).
> 현재 무엇이 있고 무엇을 지켜야 하는지는 [`shipped.md`](shipped.md).
>
> 남겨두는 이유: §7 의 RLS 검증 시나리오(RLS-1~12, AI-1)가 아직 쓸 만하고,
> Phase 0 설계 의도의 기록으로 가치가 있다.
> ---
