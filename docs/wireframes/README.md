...
>
> 작성일: 2026-05-24
> 상태: **⚠️ 아카이브(낡음, 2026-07-25 확인)** — v0.1 초안에서 멈춰 있고 **실제 화면이 훨씬 앞서 있다.**
>   당초 "Phase 1.4~1.7 마친 뒤 실제 화면 기준으로 일괄 재정비" 계획이었으나 그 재정비는 하지 않았고,
>   그 사이 회원 앱·관리자 앱·영상·코칭 큐 등 여기 없는 화면이 대거 추가됐다.
>   **UI 를 확인할 땐 이 문서가 아니라 `src/app/lib/features/` 의 실제 화면을 볼 것.**
>   남겨두는 이유: 여러 Dart 파일이 `docs/wireframes/…` 를 최초 설계 출처로 주석에 달고 있다.
>   (현재 화면 목록은 [`docs/shipped.md`](../shipped.md) §1)
