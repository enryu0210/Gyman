...
> ✅ **0001~0041 전부 Supabase SQL Editor 적용 완료**
> (0001~0034: 2026-07-06 / 0035~0039: 2026-07-25 / 0040·0041: 2026-08-10).
>
> ⚠ **0041 적용 후에는 앱을 반드시 재빌드·재설치할 것.** `member_profiles` 에 BEFORE
> INSERT 트리거가 걸려 동의 확인 없는 회원 등록을 거부한다 → **구버전 앱은 [회원 추가]가
> 실패한다**(그 컬럼을 안 보내므로). 기기에 깔린 앱이 `0041` 이전 빌드라면 지금이 그 상태다.
> 다른 기능은 영향 없다 — 트리거가 INSERT 전용이라 기존 회원 수정·조회는 그대로 동작한다.
>
